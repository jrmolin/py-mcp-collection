...
class SemanticChunker:
    ...
    def split_sentences(self, sentences: list[str]) -> list[list[str]]:
        # CUSTOM CHANGE TO LIBRARY TO TAKE PRE-SPLIT SENTENCES
        single_sentences_list = sentences
        ...
        if self.breakpoint_threshold_type == "gradient" and len(single_sentences_list) == 2:
            return [single_sentences_list]
        distances, sentences = self._calculate_sentence_distances(single_sentences_list)  # type: ignore
