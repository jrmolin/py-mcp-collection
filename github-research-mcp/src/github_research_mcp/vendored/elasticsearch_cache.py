...
class ElasticsearchCache(CacheProtocol):
    ...
    async def get_entry(self, collection: str, key: str) -> CacheEntryTypes | None:
        ...
        elasticsearch_response = await self.elasticsearch_client.options(ignore_status=404).get(index=self.index, id=collection_key)

        if elasticsearch_response.body is None or elasticsearch_response.body.get("error") or not elasticsearch_response.body.get("found"):
            return None
    ...
    async def delete(self, collection: str, key: str) -> None:
        ...
        await self.elasticsearch_client.options(ignore_status=404).delete(index=self.index, id=collection_key)
    ...
    async def setup(self) -> None:
        ...
        async with self.setup_lock:
            ...
            if await self.elasticsearch_client.options(ignore_status=404).indices.exists(index=self.index):
                return
    ...
    async def cull(self) -> None:
        await self.elasticsearch_client.options(ignore_status=404).delete_by_query(
            ...
            body={
                "query": {
                    "range": {
                        "expires_at": {"lt": datetime.now(tz=UTC).timestamp()},
                    },
                },
            },
        )
