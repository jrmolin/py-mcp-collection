...
try:
    ...
    test = embedding_model._model.embed(["Hello, world!"])
    fastembed_available = True
except Exception:
    fastembed_available = False


def join_content(nodes: list[TextNode]) -> str:
...
@pytest.fixture
def warsaw_nodes(embed_model: FastEmbedEmbedding) -> list[TextNode]:
    nodes = [
        TextNode(
...
@pytest.fixture
def football_nodes(embed_model: FastEmbedEmbedding) -> list[TextNode]:
    nodes = [
        TextNode(
...
@pytest.fixture
def mathematics_nodes(embed_model: FastEmbedEmbedding) -> list[TextNode]:
    nodes = [
        TextNode(
...
async def test_returns_all_nodes(embed_model: FastEmbedEmbedding, common_nodes: list[TextNode]) -> None:
    parser = SemanticMergerNodeParser(
    ...
    nodes = await parser.aget_nodes_from_documents(documents=common_nodes)  # type: ignore
    assert len(nodes) == len(common_nodes)


@pytest.fixture
def semantic_merger(embed_model: FastEmbedEmbedding) -> SemanticMergerNodeParser:
...
@pytest.mark.skipif(not fastembed_available, reason="FastEmbed model not available")
async def test_warsaw_nodes(semantic_merger: SemanticMergerNodeParser, warsaw_nodes: list[TextNode]) -> None:
    target_content = join_content(warsaw_nodes)

    merged_nodes = await semantic_merger.aget_nodes_from_documents(documents=warsaw_nodes)  # type: ignore

    assert len(merged_nodes) == 1
    assert merged_nodes[0].get_content() == target_content, "Warsaw nodes were not correctly merged"
...
@pytest.mark.skipif(not fastembed_available, reason="FastEmbed model not available")
async def test_football_nodes(semantic_merger: SemanticMergerNodeParser, football_nodes: list[TextNode]) -> None:
    target_content = join_content(football_nodes)

    merged_nodes = await semantic_merger.aget_nodes_from_documents(documents=football_nodes)  # type: ignore
...
@pytest.mark.skipif(not fastembed_available, reason="FastEmbed model not available")
async def test_mathematics_nodes(semantic_merger: SemanticMergerNodeParser, mathematics_nodes: list[TextNode]) -> None:
    target_content = join_content(mathematics_nodes)

    merged_nodes = await semantic_merger.aget_nodes_from_documents(documents=mathematics_nodes)  # type: ignore
...
@pytest.mark.skipif(not fastembed_available, reason="FastEmbed model not available")
async def test_combination_of_nodes(semantic_merger: SemanticMergerNodeParser, common_nodes: list[TextNode]) -> None:
