...
class HuggingFaceEmbedding(MultiModalEmbedding):
    ...
    query_instruction: Optional[str] = Field(description="Instruction to prepend to query text.", default=None)
    text_instruction: Optional[str] = Field(description="Instruction to prepend to text.", default=None)
    cache_folder: Optional[str] = Field(description="Cache folder for Hugging Face files.", default=None)
    show_progress_bar: bool = Field(description="Whether to show a progress bar.", default=False)
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        for variable, value in [
            ...
            if value != "deprecated":
                raise ValueError(f"{variable} is deprecated. Please remove it from the arguments.")
    ...
    def _embed_with_retry(
        ...
        try:
            if self._parallel_process:
                pool = self._model.start_multi_process_pool(target_devices=self._target_devices)
...
class HuggingFaceInferenceAPIEmbedding(BaseEmbedding):  # type: ignore[misc]
    ...
    query_instruction: Optional[str] = Field(default=None, description="Instruction to prepend during query embedding.")
    text_instruction: Optional[str] = Field(default=None, description="Instruction to prepend during text embedding.")
    ...
    cookies: Dict[str, str] = Field(default=None, description="Additional cookies to send to the server.")
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if kwargs.get("model_name") is None:
            ...
            logger.debug(f"Using Hugging Face's recommended model {kwargs['model_name']} given task {task}.")
    ...
    async def _async_embed_single(self, text: str) -> Embedding:
        ...
        try:
            ...
            raise ValueError(
                f"Pooling is required for {self.model_name} because it returned a > 1-D value, please specify pooling as not None."
            ) from exc
    ...
    async def _aget_query_embedding(self, query: str) -> Embedding:
        return await self._async_embed_single(text=format_query(query, self.model_name, self.query_instruction))

    async def _aget_text_embedding(self, text: str) -> Embedding:
        return await self._async_embed_single(text=format_text(text, self.model_name, self.text_instruction))

    async def _aget_text_embeddings(self, texts: List[str]) -> List[Embedding]:
        return await self._async_embed_bulk(texts=[format_text(text, self.model_name, self.text_instruction) for text in texts])
