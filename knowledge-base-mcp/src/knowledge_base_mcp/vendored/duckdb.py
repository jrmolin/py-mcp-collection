...
class DuckDBVectorStore(BasePydanticVectorStore):
    ...
    def table(self) -> duckdb.DuckDBPyRelation:
        ...
        if self._table is None:
            self._table = self._initialize_table(self.client, self.table_name, self.embed_dim)

        return self._table

    @classmethod
    def _initialize_table(cls, conn: duckdb.DuckDBPyConnection, table_name: str, embed_dim: Optional[int]) -> duckdb.DuckDBPyRelation:
    ...
    def _node_to_arrow_row(self, node: BaseNode) -> dict:
        return {
            ...
            "metadata_": node_to_metadata_dict(node, remove_text=True, flat_metadata=self.flat_metadata),
        }

    def _arrow_row_to_node(self, row_dict: dict) -> BaseNode:
        return metadata_dict_to_node(metadata=json.loads(row_dict["metadata_"]), text=row_dict["text"])
    ...
    def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:  # noqa: ARG002
        ...
        filter_expression = self._build_metadata_filter_expressions(metadata_filters=query.filters)
    ...
    def delete(self, ref_doc_id: str, **delete_kwargs: Any) -> None:  # noqa: ARG002
        ...
        """
        where_clause = self._build_metadata_filter_expression("ref_doc_id", ref_doc_id, FilterOperator.EQ)
    ...
    def _build_node_id_metadata_filter_expression(
        ...
        if filters is not None:
            filter_expression = self._build_metadata_filter_expressions(metadata_filters=filters)
    ...
    def _build_metadata_filter_expression(self, key: str, value: Any, operator: FilterOperator) -> Expression:
    ...
    def _build_filter_expression(self, column: Expression, value: Expression, operator: FilterOperator) -> Expression:
        """
        ...
        if operator == FilterOperator.IS_EMPTY:
            ...
            return column.isnull().__or__(
                CaseExpression(
                    condition=FunctionExpression("typeof", column).__eq__(ConstantExpression("ARRAY")),
                    value=FunctionExpression("length", column).__eq__(ConstantExpression(0)),
                )
            )

        raise NotImplementedError(f"Unsupported operator: {operator}")

    def _build_metadata_filter_expressions(self, metadata_filters: Optional[MetadataFilters] = None) -> Expression:
        ...
        for metadata_filter in metadata_filters.filters:
            if isinstance(metadata_filter, MetadataFilter):
                ...
                expressions.append(self._build_metadata_filter_expressions(metadata_filter))
            else:
                raise NotImplementedError(f"Unsupported metadata filter: {metadata_filter}")
        ...
        for expression in expressions[1:]:
            ...
            raise NotImplementedError(f"Unsupported condition: {metadata_filters.condition}")
