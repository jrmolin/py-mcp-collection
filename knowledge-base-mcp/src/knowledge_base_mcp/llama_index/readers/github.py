...
from llama_index.core.schema import Document, MediaResource, NodeRelationship
...
class GithubIssueCommentsReader(BasePydanticReader):
    owner: str = Field(description="The owner of the GitHub repository.")
    repo: str = Field(description="The name of the GitHub repository.")

    client: GitHub[Any] = Field(
        default_factory=lambda: GitHub(TokenAuthStrategy(token=os.environ["GITHUB_TOKEN"])), description="The GitHub client."
    )

    def _comment_to_document(self, issue_number: int, comment: IssueComment) -> Document:
    ...
    @override
    def load_data(self, issue_number: int, since: datetime | None = None, per_page: int = 100, page: int = 1) -> list[Document]:
        return list(self.lazy_load_data(issue_number, since, per_page, page))

    @override
    def lazy_load_data(
        self, issue_number: int, since: datetime | None = None, per_page: int = 100, page: int = 1
        ...
        for comment in paginator:
            yield self._comment_to_document(issue_number, comment)

    @override
    async def aload_data(self, issue_number: int, since: datetime | None = None, per_page: int = 100, page: int = 1) -> list[Document]:
        return [doc async for doc in self.alazy_load_data(issue_number, since, per_page, page)]
...
class GithubIssuesReader(BasePydanticReader):
    owner: str = Field(description="The owner of the GitHub repository.")
    repo: str = Field(description="The name of the GitHub repository.")
    ...
    @cached_property
    def comments_reader(self) -> GithubIssueCommentsReader:
        return GithubIssueCommentsReader(owner=self.owner, repo=self.repo, client=self.client)
    ...
    def _issue_to_document(self, issue: Issue) -> Document:
        ...
        if milestone := issue.milestone:
            ...
            metadata["milestone.due_on"] = milestone.due_on.isoformat() if milestone.due_on else None
    ...
    def lazy_load_data(
        ...
        *,
        ...
        for issue in paginator:
            issue_document: Document = self._issue_to_document(issue)

            issue_document_children = issue_document.relationships[NodeRelationship.CHILD] = []

            if issue.comments > 0 and include_comments:
                for comment in self.comments_reader.lazy_load_data(issue_number=issue.number):
                    comment.relationships[NodeRelationship.PARENT] = issue_document.as_related_node_info()
                    issue_document_children.append(comment.as_related_node_info())

                    comment.metadata["issue.title"] = issue.title

                    yield comment

            yield issue_document
    ...
    async def alazy_load_data(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        *,
        ...
        async for issue in paginator:
            ...
            if issue.comments > 0 and include_comments:
                async for comment in self.comments_reader.alazy_load_data(issue_number=issue.number):
                    comment.relationships[NodeRelationship.PARENT] = issue_document.as_related_node_info()
