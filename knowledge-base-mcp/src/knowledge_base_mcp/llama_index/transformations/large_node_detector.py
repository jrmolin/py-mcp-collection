...
class LargeNodeDetector(TransformComponent):
    ...
    def from_embed_model(
        cls, embed_model: BaseEmbedding, node_type: Literal["leaf", "all"] = "all", exclude: bool = False
    ) -> "LargeNodeDetector":
        ...
        return cls(max_size=max_size, node_type=node_type, exclude=exclude)
    ...
    def __call__(self, nodes: Sequence[BaseNode], **kwargs: Any) -> Sequence[BaseNode]:  # pyright: ignore[reportAny]
        ...
        for node in nodes:
            ...
            node_is_leaf: bool = node.child_nodes is None or len(node.child_nodes) == 0
            ...
            if content_size > self.max_size and node_type_matches:
                ...
                logger.warning(msg=f"{action} Node {node.id_} -- size {content_size} > max size {self.max_size}: {content[:100]}...")
