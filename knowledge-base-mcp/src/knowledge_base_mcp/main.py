...
from knowledge_base_mcp.servers.documentation.manage import KnowledgeBaseManagementServer
from knowledge_base_mcp.servers.documentation.search import KnowledgeBaseSearchServer
from knowledge_base_mcp.servers.ingest import IngestServer
...
class Store(BaseKBArbitraryModel):
...
class PartialCliContext(BaseKBArbitraryModel):
...
class CliContext(BaseKBArbitraryModel):
    docs_stores: Store
    code_stores: Store
...
DEFAULT_DOCS_EMBEDDINGS_BATCH_SIZE = 48
...
DEFAULT_CODE_EMBEDDINGS_BATCH_SIZE = 48
...
def cli(
    ...
    logger.info(f"Loading document model {document_embeddings_model} and code model {code_embeddings_model} for embeddings")
...
@click.pass_context
async def duckdb_persistent(ctx: click.Context, code_db_dir: Path, code_db_name: str, docs_db_dir: Path, docs_db_name: str) -> None:
    from knowledge_base_mcp.stores.vector_stores.duckdb import EnhancedDuckDBVectorStore
    from knowledge_base_mcp.vendored.storage.docstore.duckdb import DuckDBDocumentStore
    from knowledge_base_mcp.vendored.storage.index_store.duckdb import DuckDBIndexStore
    from knowledge_base_mcp.vendored.storage.kvstore.duckdb import DuckDBKVStore
    ...
    logger.info(f"Loading DuckDB document and code stores in persistent mode: {code_db_dir / code_db_name}")
...
async def run(ctx: click.Context):
    ...
    kbmcp: FastMCP = FastMCP(name="Knowledge Base MCP")  # pyright: ignore[reportMissingTypeArgument, reportUnknownVariableType]

    knowledge_base_client: KnowledgeBaseClient = KnowledgeBaseClient(vector_store_index=cli_ctx.docs_stores.vector_store_index)

    ingest_server: IngestServer = IngestServer(knowledge_base_client=knowledge_base_client)

    [kbmcp.add_tool(tool) for tool in ingest_server.get_raw_tools()]
    ...
    [kbmcp.add_tool(tool) for tool in kb_management_server.get_raw_tools()]

    search_server: KnowledgeBaseSearchServer = KnowledgeBaseSearchServer(
