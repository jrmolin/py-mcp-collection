...
from filesystem_operations_mcp.filesystem.utils.ripgrep import RipGrepFind, RipGrepSearch, RipGrepSearchResult
...
class TestRipGrepSearch:
    ...
    def test_benchmark_ripgrep_search_direct(self, ripgrep_benchmark: RipGrepSearch, benchmark: BenchmarkFixture):
        _ = ripgrep_benchmark.add_pattern("la")

        def run_search():
            result = list(ripgrep_benchmark.run_direct())

            assert len(result) == 163575

        benchmark(run_search)

    def test_benchmark_ripgrep_search(self, ripgrep_benchmark: RipGrepSearch, benchmark: BenchmarkFixture):
        _ = ripgrep_benchmark.add_pattern("la")

        def run_search():
            result = list(ripgrep_benchmark.run(exclude_submatches=True))

            assert len(result) == 11244
